...
class TokkoInactiveProperty {
   var $BASE_URL = "http://www.tokkobroker.com/api/v1/inactiveproperty/";
   function __construct ($date, $auth=null){
     try {
       $url = $this->BASE_URL  . "?format=json&key=". $auth->key ."&lang=".$auth->get_language()."&deleted_at__gte=".$date;
       $cp = curl_init();
       curl_setopt($cp, CURLOPT_RETURNTRANSFER, 1);
       curl_setopt($cp, CURLOPT_URL, $url);
       curl_setopt($cp, CURLOPT_TIMEOUT, 60);
   ...
   function get_properties() {
     $res = [];
     foreach ($this->data->objects as $row) {
       $res[] = $row->id;
     }
     return $res;
   }
